...
- [ ] CHK013 - Are kernel sizes and filter parameters for edge detection specified? **FAIL** — Spec §FR-001 mentions "normalized Canny edge map" and "normalized sliding-window variance" but does not specify kernel sizes, Gaussian σ, or window dimensions. MEMORY.md line 72 records research findings (Gaussian 5×5 σ=1.4, density window 7×7 to 31×31) but these are not in the spec. [Gap, Spec §FR-001]
- [ ] CHK014 - Is the normalization method for the combined heatmap specified? **FAIL** — Spec §FR-001 says "normalized" but does not specify min-max normalization, z-score, or other method. No normalization formula is given. [Gap, Spec §FR-001]
...
- [ ] CHK026 - Are the sigmoid temperature parameters β₀, β₁, β₂ defined with defaults and ranges? **FAIL** — Spec §FR-003 says "β₀, β₁, β₂ are configurable temperature/bias parameters" but provides no default values and no valid ranges. A developer cannot initialize these without guessing. [Clarity, Spec §FR-003]
...
- [ ] CHK038 - Are batch size, crop size, and augmentation parameters documented? **FAIL** — Crop size (512×512) and augmentation (spatial-only) are documented in FR-007 and FR-026. Batch size is NOT explicitly stated anywhere in the spec. MEMORY.md line 278 mentions "batch size 1 (source + target)" but this is not in the spec itself. [Completeness, Spec §FR-007, FR-026]
...
- [ ] CHK043 - Is the expected magnitude of improvement discussed? **FAIL** — Spec says SATG should "achieve higher mIoU" but does not discuss expected magnitude (e.g., "2-5 mIoU improvement"). No discussion of what would constitute a meaningful vs. marginal improvement. [Gap, Spec §Success Criteria]
...
- [ ] CHK052 - RISK-02 (coarse prior): Is kernel size ablation included? **FAIL** — Spec §User Story 9 includes "ablation on prior type" (edge-only, variance-only, combined) but does NOT include kernel size ablation (e.g., Gaussian 3×3 vs 5×5 vs 7×7, or density window 7×7 vs 15×15 vs 31×31). This is a gap for validating the structural prior's spatial scale sensitivity. [Gap, Spec §User Story 9]
...
- [ ] CHK055 - Are mitigation strategies for each identified risk documented? **FAIL** — The spec has no explicit "Risk Mitigation" section. Edge cases are addressed, but systematic risk identification with corresponding mitigations is absent. The risks listed in this checklist (complex-class bias, coarse prior, hyperparameter sensitivity, augmentation mismatch) are not formally documented in the spec with mitigation plans. [Gap, Spec §Risk Coverage]
